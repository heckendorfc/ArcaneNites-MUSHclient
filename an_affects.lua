...
function show_affects()
	...
	aff_winheight = (winpos.bottom*0.8)-vit_winheight-(WIN_GAP)
	...
	check (WindowCreate (affects_win, 
		...
		aff_winwidth-WIN_GAP,
		aff_winheight,
		8,       -- auto-position: top middle
		2,  -- flags
		BACKGROUND_COLOUR) )

	check (WindowRectOp (affects_win,1,0,0,aff_winwidth-WIN_GAP,aff_winheight,BORDER_COLOUR))
	add_drag_properties(affects_win)

	-- line = string.format("%s: %s",aff[1][1],aff[1][2])
	-- Display_Line (affects_win, 1, line, font_id, text_colour)

	Display_Line (affects_win, 1, "\27[1;36mAffects:", font_id, text_colour)

	for i=1,#aff do
		if aff[i][1] ~= nil then
			line = string.format ("%s: %s", aff[i][1], aff[i][2])
			Display_Line (affects_win, i+2, line, font_id, text_colour)
		end
	end
